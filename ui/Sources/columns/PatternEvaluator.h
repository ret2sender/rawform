...
// PatternEvaluator.h
//
// The per-field renderer and the custom-column %pattern% evaluator.
//
...
//
// Scope: %token% substitution + literal text only. A full title-formatting
// language ($if, $functions, [...] sections) is outside this evaluator's
// contract.
...
/// Render one built-in field of a track as a display string. Direct tag/audio/filesystem
/// reads plus the two composites (TrackIndex -> "1.01", AlbumGroup -> "Album Artist -
/// Album"). Pure.
[[nodiscard]] QString renderFieldValue(const TrackData& track, ColumnField field);

/// Optional per-token value hook for evaluatePattern (file rename).
///
...
/// Evaluate a custom-column pattern (literal text interleaved with %token% references)
/// against a track.
///
