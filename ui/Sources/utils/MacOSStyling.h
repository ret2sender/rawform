...
/// @file MacOSStyling.h
/// @brief Native macOS window styling for the rawform application.
///
...
/// @c \<QQmlApplicationEngine\> header is pulled in only by the .mm
...
/// @brief Applies native macOS window styling to the application's root window.
///
...
/// @c startSystemMove() instead), and measures the native title bar height so
...
/// and main.cpp only calls it under @c Q_OS_MAC, so the no-op branch is a
/// belt-and-suspenders guard rather than a path the app relies on.
///
/// @param engine The QQmlApplicationEngine whose root window will be styled.
